...
def score_answers(answers: dict[str, int]) -> dict[str, float]:
    """
    answers: {question_id: option_index}.
    Returns a sanitized trait vector. Missing/invalid answers are skipped.
    """
...
def public_questions() -> list[dict]:
    """Questionnaire without the trait deltas (what the client renders)."""
    return [
        {
            "id": q["id"],
            "prompt": q["prompt"],
